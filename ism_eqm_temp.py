...
def dust_temperature(nH=1,T=10,Z=1,NH=0, ISRF=1, z=0,beta=2):
    func = lambda logT: net_dust_heating(10**logT, nH,T,NH,Z,ISRF,z,beta)
#    return brentq(net_dust_heating,1,10**5,args=(nH,T,NH,Z,ISRF,T_CMB))
    return 10**brentq(func,-1,8)

dust_temperature = np.vectorize(dust_temperature)

def net_dust_heating(Td,nH,T,NH,Z=1,ISRF=1,z=0, beta=2):
    T_CMB = 2.73*(1+z)
    ...
    T_ISRF=max(20,3.8*(ISRF_OPT_eV_cm3+ISRF_IR_eV_cm3)**0.25)
    sigma_IR_ISRF = sigma_IR_0 * Z * (T_ISRF/10)**beta # assume 20K dust emission
    sigma_UV = 1e-21 * Z
    tau_d = sigma_UV * NH
    sigma_OPT = 3e-22 * Z
    lambdadust_thin = 2.268 * sigma_IR_d * (Td/10)**4
    lambdadust_thick = 2.268 * (Td/10)**4 / (NH+1e-100)
    psi_IR = 1/(1/lambdadust_thin + 1/lambdadust_thick) #min(2.268 * sigma_IR_d * (Td/10)**4, 2.268 * (Td/10)**4 / (NH+1e-100))
...
compression=0,dust_beta=2.,attenuate_cr=True,co_prescription="Whitworth 2018",cii_prescription="Hopkins 2022 (FIRE-3)"):
    ...
    #print(compression)
    rate = CR_heating(zeta_CR,NH*attenuate_cr) - lyman_cooling(T) + photoelectric_heating(ISRF, nH,T, NH, Z) - CII_cooling(nH, Z, T, NH, ISRF,prescription=cii_prescription) \
         - CO_cooling(nH,T,NH,Z,ISRF,divv,prescription=co_prescription) + dust_gas_cooling(nH,T,Tdust,Z) + compression*compression_heating(nH,T) #+ turbulent_heating()
    return rate
...
compression=False,dust_beta=2.,attenuate_cr=True,return_Tdust=True,co_prescription="Whitworth 2018",cii_prescription="Hopkins 2022 (FIRE-3)"):
    if NH==0: NH=1e18
    ...
    T = 10**brentq(func, -1,8,rtol=1e-2)
