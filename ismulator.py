...
def make_plot_bokeh(n, T, Tdust, x_var="Density", y_var="Temperature"):
    labeldict = {
        "Density": "nH (cm^-3)",  # r"$$n_{\rm H}\,\left(\rm cm^{-3}\right)$$",
    ...
    quantities = {"Density": n, "Jeans Mass": M_J, "Temperature": T, "Pressure": P}
...
NH2 = 10 ** st.sidebar.slider(
    r"Column density: $\log N_{\rm H}/\rm cm^{-2}$ at $n_{\rm H}=100\,\rm cm^{-3}$",
    min_value=19.0,
    max_value=23.0,
    value=21.0,
)
...
X_FUV = 10 ** st.sidebar.slider(
    r"$\log \chi_{\rm FUV}$: FUV radiation field strength",
    min_value=-2.0,
    max_value=4.0,
    value=0.0,
)
X_OPT = 10 ** st.sidebar.slider(
    r"$\log \chi_{\rm OPT}$: Optical-NIR radiation field strength",
    min_value=-2.0,
    max_value=4.0,
    value=0.0,
)
NH_alpha = st.sidebar.slider(
    r"Column density scaling: $\alpha$ where $N_{\rm H}=N_{\rm H,0}\left(n_{\rm H}/100\rm cm^{-3}\right)^{\alpha}$",
    min_value=0.0,
    max_value=1.0,
    value=0.3,
)
fJ = st.sidebar.slider(
    r"$f_{\rm J}=l/\lambda_{\rm J}$: Ratio of shielding length floor to Jeans wavelength",
    min_value=0.0,
    max_value=1.0,
    value=0.25,
)
...
sigma_GMC = 10 ** st.sidebar.slider(
    r"$\log \Sigma_{\rm GMC}\left(M_\odot\rm pc^{-2}\right)$ for turb. dissipation",
    min_value=1.0,
    max_value=5.0,
    value=2.0,
)
...
Tmin = 1
Tmax = 1e4
nmin = 10 ** st.sidebar.number_input(
    r"Min. $\log n_{\rm H}\left(\rm cm^{-3}\right)$",
    min_value=-2.0,
    max_value=20.0,
    value=0.0,
)
nmax = 10 ** st.sidebar.number_input(
    r"Max. $\log n_{\rm H}\left(\rm cm^{-3}\right)$",
    min_value=0.0,
    max_value=20.0,
    value=8.0,
)
...
try:
    T, Tdust = equilibrium_temp_grid(
        ...
        X_OPT=X_OPT,
        zeta_CR=zeta_CR,
        attenuate_cr=attenuate_cr,
        dust_beta=dust_beta,
        dust_coupling=dust_coeff,
        sigma_GMC=sigma_GMC,
        co_prescription=CO_Rx,
        cii_prescription=CI_Rx,
        processes=processes_to_use,
        return_Tdust=True,
    )
except:
